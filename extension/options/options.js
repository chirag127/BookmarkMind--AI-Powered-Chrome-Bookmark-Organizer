...
class OptionsController {
  constructor() {
    ...
    chrome.runtime.onMessage.addListener((message) => {
      if (message.type === 'LEARNING_DATA_UPDATED') {
        ...
        this.loadLearningData();
        this.showToast(
          `Learned ${message.count} new patterns from bookmark move`,
          'success'
        );
      }
    });
  ...
  initializeElements() {
    ...
    this.viewModelComparison = document.getElementById(
      'viewModelComparison'
    );
    ...
    this.toggleCerebrasApiKeyBtn = document.getElementById(
      'toggleCerebrasApiKey'
    );
    this.clearCerebrasApiKeyBtn = document.getElementById(
      'clearCerebrasApiKey'
    );
    ...
    this.cerebrasApiKeyStatus = document.getElementById(
    ...
    this.cleanupEmptyFoldersCheckbox = document.getElementById(
      'cleanupEmptyFolders'
    );
    this.maxCategoryDepthSlider =
            document.getElementById('maxCategoryDepth');
    this.maxDepthValueDisplay = document.getElementById('maxDepthValue');
    this.minBookmarksThresholdSlider = document.getElementById(
      'minBookmarksThreshold'
    );
    ...
    this.organizedBookmarksCount = document.getElementById(
      'organizedBookmarksCount'
    );
    this.categoriesCount = document.getElementById('categoriesCount');
    this.learningPatternsCount = document.getElementById(
      'learningPatternsCount'
    );
    this.lastSortDate = document.getElementById('lastSortDate');

    // Data management elements
    this.consolidateFoldersBtn =
            document.getElementById('consolidateFolders');
    this.exportDataBtn = document.getElementById('exportData');
    this.clearLearningDataBtn =
            document.getElementById('clearLearningData');
    this.resetSettingsBtn = document.getElementById('resetSettings');

    // Learning data elements
    this.totalLearningPatterns = document.getElementById(
      'totalLearningPatterns'
    );
    this.lastLearningUpdate = document.getElementById('lastLearningUpdate');
    this.learningPatternsList = document.getElementById(
      'learningPatternsList'
    );
    this.noPatternsMessage = document.getElementById('noPatternsMessage');
    this.refreshLearningDataBtn = document.getElementById(
      'refreshLearningData'
    );
    this.exportLearningDataBtn =
            document.getElementById('exportLearningData');
    this.importLearningDataBtn =
            document.getElementById('importLearningData');

    // Performance monitoring elements
    this.avgCategorizationTime = document.getElementById(
      'avgCategorizationTime'
    );
    this.performanceSuccessRate = document.getElementById(
    ...
    this.providerComparisonChart = document.getElementById(
    ...
    this.performanceHistoryChart = document.getElementById(
    ...
    this.refreshPerformanceData = document.getElementById(
  ...
  attachEventListeners() {
    ...
    this.testGeminiKeyBtn.addEventListener('click', () =>
      this.testGeminiKey()
    );
    ...
    this.testCerebrasKeyBtn.addEventListener('click', () =>
      this.testCerebrasKey()
    );
    this.saveCerebrasKeyBtn.addEventListener('click', () =>
      this.saveCerebrasKey()
    );

    // Groq API Key events
    this.groqApiKeyInput.addEventListener('input', () =>
      this.onGroqApiKeyChange()
    );
    this.toggleGroqApiKeyBtn.addEventListener('click', () =>
      this.toggleGroqApiKeyVisibility()
    );
    this.clearGroqApiKeyBtn.addEventListener('click', () =>
    ...
    // Settings events
    this.batchSizeSelect.addEventListener('change', () =>
      this.saveSettings()
    );
    this.cleanupEmptyFoldersCheckbox.addEventListener('change', () =>
      this.saveSettings()
    );
    this.maxCategoryDepthSlider.addEventListener('input', () =>
      this.onMaxDepthChange()
    );
    this.maxCategoryDepthSlider.addEventListener('change', () =>
      this.saveSettings()
    );
    this.minBookmarksThresholdSlider.addEventListener('input', () =>
      this.onMinThresholdChange()
    );
    this.minBookmarksThresholdSlider.addEventListener('change', () =>
      this.saveSettings()
    );
    ...
    this.refreshLearningDataBtn.addEventListener('click', () =>
      this.loadLearningData()
    );
    if (this.exportLearningDataBtn) {
      this.exportLearningDataBtn.addEventListener('click', () =>
        this.exportLearningData()
      );
    }
    if (this.importLearningDataBtn) {
      this.importLearningDataBtn.addEventListener('click', () =>
    ...
    if (this.refreshPerformanceData) {
      this.refreshPerformanceData.addEventListener('click', () =>
        this.loadPerformanceData()
      );
    }
    if (this.setLast30Days) {
      this.setLast30Days.addEventListener('click', () =>
        this.setDateRangeLast30Days()
      );
    }
    if (this.setAllTime) {
      this.setAllTime.addEventListener('click', () =>
        this.setDateRangeAllTime()
      );
    }
    if (this.exportReportJSON) {
      this.exportReportJSON.addEventListener('click', () =>
        this.exportReport('json')
      );
    }
    if (this.exportReportCSV) {
      this.exportReportCSV.addEventListener('click', () =>
  ...
  async loadSettings() {
    try {
      ...
      this.settings =
                result.bookmarkMindSettings || this.getDefaultSettings();
  ...
  updateSettingsUI() {
    ...
    this.maxDepthValueDisplay.textContent =
            this.settings.maxCategoryDepth || 2;
  ...
  updateStatsUI() {
    ...
    this.learningPatternsCount.textContent =
            this.stats.learningPatterns || 0;
  ...
  async exportLearningData() {
    try {
      ...
      if (response && response.success) {
        ...
        a.href = url;
        a.download = `bookmarkmind-learning-${new Date()
          .toISOString()
          .slice(0, 10)}.json`;
        ...
        this.showToast(
          'Learning data exported successfully',
          'success'
        );
      } else {
      ...
      input.onchange = async (e) => {
        ...
        reader.onload = async (event) => {
          try {
            ...
            if (response && response.success) {
              this.showToast(
                `Imported ${response.data.patternsCount} patterns`,
                'success'
              );
              this.loadLearningData();
              this.loadStats();
            ...
            this.showToast(
              'Failed to import learning data: Invalid file format',
              'error'
            );
          }
        };
    ...
    try {
      ...
      const timeoutPromise = new Promise((_, reject) => {
        setTimeout(
          () =>
            reject(
              new Error('API key test timeout after 15 seconds')
            ),
          15000
        );
      });
      ...
      if (response && response.success && response.valid) {
        this.showApiKeyStatus(
          '✓ Gemini API key is valid! Saving...',
          'success'
        );
      ...
      this.showApiKeyStatus(
        `Failed to test API key: ${_error.message}`,
        'error'
      );
    } finally {
  ...
  async testCerebrasKey() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showCerebrasApiKeyStatus(
          '✓ Cerebras API key is valid! Saving...',
          'success'
        );
        ...
        this.showToast(
          'Cerebras API key validated and saved!',
          'success'
        );
  ...
  showCerebrasApiKeyStatus(message, type) {
    this.cerebrasApiKeyStatus.classList.remove(
      'hidden',
      'success',
      'error',
      'loading'
  ...
  toggleGroqApiKeyVisibility() {
    if (
      ...
      this.showToast(
        'Please clear the key first to enter a new one',
        'info'
      );
      return;
    }
    ...
    this.groqApiKeyInput.type = this.isGroqApiKeyVisible
      ? 'text'
      : 'password';
  ...
  async testGroqKey() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.showToast(
          `Groq API key validation failed: ${errorMsg}`,
          'error'
        );
      }
    } catch (_error) {
  ...
  async saveGroqKey() {
    ...
    if (!apiKey.startsWith('gsk_') || apiKey.length < 10) {
      this.showGroqApiKeyStatus(
        'Invalid API key format. Key should start with "gsk_"',
        'error'
      );
      return;
    }
  ...
  showGroqApiKeyStatus(message, type) {
    this.groqApiKeyStatus.classList.remove(
      'hidden',
      'success',
      'error',
      'loading'
  ...
  onNewCategoryChange() {
    ...
    this.addCategoryBtn.disabled =
            !value || this.settings.categories.includes(value);
  }
  ...
  async saveSettings() {
    try {
      ...
      this.settings.cleanupEmptyFolders =
                this.cleanupEmptyFoldersCheckbox.checked;
      this.settings.maxCategoryDepth = parseInt(
        this.maxCategoryDepthSlider.value
      );
      this.settings.minBookmarksThreshold = parseInt(
        this.minBookmarksThresholdSlider.value
      );
  ...
  async consolidateFolders() {
    try {
      ...
      if (preview.totalFoldersToRemove === 0) {
        this.showToast(
          'No sparse folders found to consolidate',
          'info'
        );
        return;
      }
      ...
      if (!consolidateResponse || !consolidateResponse.success) {
        throw new Error(
          consolidateResponse?.error || 'Consolidation failed'
        );
      }
      ...
      this.showToast(
        'Folder consolidation completed successfully!',
        'success'
      );
  ...
  async exportAllData() {
    try {
      ...
      const exportData = {
        ...
        learningData: learningResponse.success
      ...
      this.showToast(
        'Settings and learning data exported successfully!',
        'success'
      );
    } catch (_error) {
  ...
  displayPerformanceDashboard(dashboard) {
    ...
    if (this.avgCategorizationTime) {
      this.avgCategorizationTime.textContent =
                dashboard.overview.avgCategorizationTime || '-';
    }
    ...
    if (this.totalApiCalls) {
      this.totalApiCalls.textContent =
                dashboard.overview.totalProcessed || 0;
    }
    if (this.memoryUsage && dashboard.currentMemory) {
      this.memoryUsage.textContent =
                dashboard.currentMemory.usedMB || '-';
    }
    ...
    if (dashboard.batchEfficiency) {
      if (this.avgBatchSize) {
        this.avgBatchSize.textContent =
                    dashboard.batchEfficiency.avgBatchSize || '-';
      }
      if (this.avgBatchTime) {
        this.avgBatchTime.textContent =
                    dashboard.batchEfficiency.avgBatchTime || '-';
      }
      if (this.avgTimePerItem) {
        this.avgTimePerItem.textContent =
                    dashboard.batchEfficiency.avgTimePerItem || '-';
      }
      if (this.efficiencyScore) {
        this.efficiencyScore.textContent =
                    dashboard.batchEfficiency.efficiencyScore || '-';
      }
    }
  ...
  displayProviderComparison(providerData) {
    ...
    const sortedProviders = providers.sort(
      (a, b) =>
        providerData[a].avgResponseTime -
                providerData[b].avgResponseTime
    );
  ...
  displayPerformanceHistory(historyData) {
    ...
    if (times.length > 0) {
      ...
      times.forEach((time, index) => {
        const x =
                    padding + (graphWidth / (times.length - 1 || 1)) * index;
        const y =
                    canvas.height - padding - (time / maxTime) * graphHeight;
    ...
    dates.forEach((date, index) => {
      if (index % labelInterval === 0 || index === dates.length - 1) {
        const x =
                    padding + (graphWidth / (dates.length - 1 || 1)) * index;
        ctx.save();
  ...
  async exportReport(format) {
    try {
      ...
      if (response && response.success) {
        ...
        this.showToast(
          `Analytics report exported as ${format.toUpperCase()}`,
          'success'
        );
      } else {
