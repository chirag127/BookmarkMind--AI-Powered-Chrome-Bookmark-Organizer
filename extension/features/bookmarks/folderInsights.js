...
export class FolderInsights {
    ...
    async getFolderStats(folderId) {
        ...
        const stats = {
            ...
            lastModified:
                folderNode.dateGroupModified || folderNode.dateAdded || 0,
            aiConfidenceScores: [],
            averageConfidence: 0,
            bookmarks: [],
            subfolders: [],
    ...
    calculateHealthScore(folderStats) {
        const scores = {
            bookmarkDistribution: this._scoreBookmarkDistribution(
    ...
    async getSmartSuggestions(folderStats) {
        ...
        if (
            folderStats.averageConfidence < 0.6 &&
            folderStats.averageConfidence > 0
        ) {
            suggestions.push({
                type: "confidence",
                priority: "medium",
                ...
                description:
                    "Some bookmarks may be miscategorized. Review and recategorize if needed.",
                action: "review",
                folderId: folderStats.id,
            });
        }
    ...
    async _getAIMetadata(bookmarks) {
        ...
        return {
            scores,
            average:
                bookmarks.length > 0 ? totalConfidence / bookmarks.length : 0,
        };
    }
    ...
    _scoreBookmarkDistribution(count) {
        if (
            count >= this.IDEAL_BOOKMARKS_MIN &&
            count <= this.IDEAL_BOOKMARKS_MAX
        ) {
            return 100;
        }
    ...
    _scoreOrganization(folderStats) {
        ...
        if (
            folderStats.subfolderCount > 0 &&
            folderStats.directBookmarkCount > 0
        ) {
            score += 15;
        }
    ...
    _generateRecommendations(folderStats, scores) {
        ...
        if (scores.bookmarkDistribution < 70) {
            if (folderStats.directBookmarkCount > this.IDEAL_BOOKMARKS_MAX) {
                recommendations.push(
                    "Consider splitting this folder into subcategories"
                );
            } else if (
                folderStats.directBookmarkCount < this.IDEAL_BOOKMARKS_MIN
            ) {
                recommendations.push(
                    "Consider merging this folder with related folders"
                );
            }
        }
        ...
        if (scores.depthAppropriate < 70) {
            recommendations.push(
                "Folder hierarchy is too deep - consider flattening"
            );
        }

        if (scores.organizationQuality < 70) {
            recommendations.push(
                "Improve organization by balancing bookmarks and subfolders"
            );
        }

        if (scores.aiConfidence < 60) {
            recommendations.push(
                "Review bookmarks with low AI confidence scores"
            );
        }
    ...
    _findBestOrganized(comparisons) {
        ...
        return comparisons.reduce((best, current) => {
            return current.health.totalScore > best.health.totalScore
                ? current
                : best;
        });
    }
