...
function initializeEventListeners() {
    ...
    document
        ...
        .addEventListener("click", handleRefreshStats);
    document
        .getElementById("compareBtn")
        .addEventListener("click", handleCompare);
    document
        .getElementById("generateTreemapBtn")
        .addEventListener("click", handleGenerateTreemap);
}
    ...
    if (node.id !== "0") {
        ...
        if (node.children) {
            node.children.forEach((child) =>
                extractFolders(child, folders, folderPath)
            );
        }
...
async function loadFolderStats(folderId) {
    ...
    try {
        ...
        document
            .getElementById("folderStatsSection")
...
function displayFolderStats(stats) {
    ...
    document.getElementById("directBookmarks").textContent =
        stats.directBookmarkCount;
    document.getElementById("subfolderCount").textContent =
...
function displayHealthScore(health) {
    ...
    updateMetricBar(
        "distributionScore",
        "distributionValue",
        health.breakdown.bookmarkDistribution
    );
    updateMetricBar(
        "depthScore",
        "depthValue",
        health.breakdown.depthAppropriate
    );
    updateMetricBar(
        "organizationScore",
        "organizationValue",
        health.breakdown.organizationQuality
    );
    updateMetricBar(
        "aiConfidenceScore",
        "aiConfidenceValue",
        health.breakdown.aiConfidence
    );
}
...
function updateMetricBar(barId, valueId, score) {
    ...
    if (score >= 80) {
        ...
        bar.style.background =
            "linear-gradient(90deg, #8bc34a 0%, #cddc39 100%)";
    } else if (score >= 40) {
        bar.style.background =
...
function displayRecommendations(recommendations) {
    ...
    if (recommendations.length === 0) {
        list.innerHTML =
            "<li>No specific recommendations. Keep up the good work!</li>";
        return;
    }
...
function handleSuggestionAction(e) {
    ...
    switch (action) {
        case "split":
            alert(
                `Split folder functionality would be implemented here for folder ${folderId}`
            );
            break;
        case "consolidate":
            alert(
                `Consolidate folder functionality would be implemented here for folder ${folderId}`
            );
            break;
        case "flatten":
            alert(
                `Flatten folder functionality would be implemented here for folder ${folderId}`
            );
            break;
        case "review":
            window.location.href = "popup.html";
...
function renderTreemapNode(container, nodes, x, y, width, height) {
    const totalBookmarks = nodes.reduce(
        (sum, node) => sum + node.bookmarkCount,
        0
    );
...
async function displayFavorites(favoriteIds) {
    ...
    for (const folderId of favoriteIds) {
      ...
      `;
            ...
            console.error(
                `_error loading favorite folder ${folderId}:`,
                _error
            );
        }
    }
...
async function displayAccessStats(accessStats) {
    ...
    if (sorted.length === 0) {
        container.innerHTML =
            '<p style="text-align: center; color: #5f6368;">No access data yet</p>';
        return;
    }
